...
#
# Files can also be provided under the puppet:///modules/cron/hourly/ directory
...
#
# $purge affects the /etc/cron.d/hourly/ directory as a whole, by default this
...

{
  ...
  if is_hash($cron_jobs) == true {
    ...
    # other locations in the manifest would return the class 'cron::hourly' as
    # namevar as well as numerous other issues.

    each($cron_jobs) | $index, $value | {
      file { "/etc/cron.hourly/${index}":
        ensure  => present,
